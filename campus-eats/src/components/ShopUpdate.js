...
const ShopUpdate = () => {
  ...
  useEffect(() => {
    const fetchShopData = async () => {
      try {
        const response = await axios.get(`/api/shop/${currentUser.uid}`);
        const shopData = response.data;
        setShopName(shopData.shopName);
        setShopDesc(shopData.shopDesc);
        setShopAddress(shopData.shopAddress);
        setGoogleLink(shopData.googleLink);
        setShopOpen(shopData.shopOpen);
        setShopClose(shopData.shopClose);
        setGCASHName(shopData.GCASHName);
        setGCASHNumber(shopData.GCASHNumber);
        setUploadedImage(shopData.shopImage);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.put(`/api/shop-update/${currentUser.uid}`, formData, {
          headers: {
